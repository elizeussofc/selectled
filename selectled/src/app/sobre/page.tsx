      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-[rgba(10,10,10,0.9)] backdrop-blur-xl border-b border-[#2C2C2E]">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm text-[#A1A1A6]">
            ...
            <Link href="/sao-paulo/orcamento" className="px-4 py-1.5 bg-[#FF0000] text-white rounded-lg text-sm font-semibold hover:bg-[#FF1A0E] transition-colors">
              Orçamento
            </Link>
          </nav>
        </div>
        ...
        <section className="py-24 border-b border-[#1C1C1E]">
          <Container>
            <div className="max-w-3xl">
              <p className="text-xs font-semibold uppercase tracking-widest text-[#FF0000] mb-4">Sobre nós</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {values.map(({ icon: Icon, title, desc }) => (
                <div key={title} className="p-6 bg-[#141414] border border-[#2C2C2E] rounded-2xl">
                  <div className="p-2.5 bg-[rgba(255,0,0,0.1)] rounded-xl w-fit mb-4">
            ...
            <div className="max-w-2xl mx-auto">
              {timeline.map((t, i) => (
                <div key={t.year} className="flex gap-6 pb-8 relative">
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#1C1C1E] border-2 border-[#FF0000] flex items-center justify-center shrink-0 z-10">
                    <Calendar size={14} className="text-[#FF0000]" />
                  </div>
                  <div className="pt-1.5">
                    <span className="text-sm font-bold text-[#FF0000]">{t.year}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
              {team.map((member) => (
                <div key={member.name} className="bg-[#141414] border border-[#2C2C2E] rounded-2xl p-5 text-center">
                  <div className="w-20 h-20 rounded-full mx-auto mb-4 border-2 border-[rgba(255,0,0,0.2)] overflow-hidden bg-[#1C1C1E] flex items-center justify-center">
                    {member.photo ? (
                      // eslint-disable-next-line @next/next/no-img-element
                  ...
                  <p className="text-xs text-[#FF0000] font-medium mb-2">{member.role}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
              {cities.map((city) => (
                <Link
                  key={city.slug}
                  href={`/${city.slug}`}
                  className="flex items-center gap-2.5 p-3.5 bg-[#141414] border border-[#2C2C2E] rounded-xl hover:border-[rgba(255,0,0,0.3)] hover:bg-[rgba(255,0,0,0.04)] transition-all group"
                >
                  <MapPin size={13} className="text-[#3A3A3C] group-hover:text-[#FF0000] transition-colors shrink-0" />
                  <div>
          ...
          <Container>
            <div className="max-w-3xl mx-auto">
              <div className="text-center mb-10">
                <p className="text-xs font-semibold uppercase tracking-widest text-[#FF0000] mb-2">Ecossistema</p>
