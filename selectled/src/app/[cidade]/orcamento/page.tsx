              ...
              {[1, 2, 3].map((n) => (
                <div key={n} className="flex items-center gap-2">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                    step > n ? "bg-[#30D158] text-white" : step === n ? "bg-[#FF0000] text-white" : "bg-[#1C1C1E] text-[#6E6E73]"
                  }`}>
  ...
  return (
    <form onSubmit={handleSubmit(onNext)} className="space-y-6">
      ...
      <div>
        ...
        <select {...register("eventType")} className="w-full bg-[#141414] border border-[#2C2C2E] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#FF0000] transition-colors">
      ...
      <div>
        ...
        <input type="date" {...register("eventDate")} className="w-full bg-[#141414] border border-[#2C2C2E] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#FF0000] transition-colors" />
          ...
          {(["indoor", "outdoor", "hibrido"] as const).map((loc) => (
            <label key={loc} className="cursor-pointer">
              ...
              <div className={`text-center py-2.5 rounded-xl border text-sm font-medium transition-all ${
                watch("location") === loc ? "border-[#FF0000] bg-[rgba(255,0,0,0.08)] text-[#FF0000]" : "border-[#2C2C2E] text-[#6E6E73] hover:border-[rgba(255,255,255,0.14)]"
              }`}>
      ...
      <div>
        ...
        <input type="range" min={50} max={10000} step={50} {...register("audience", { valueAsNumber: true })} className="w-full h-1.5 rounded-full appearance-none bg-[#2C2C2E] accent-[#FF0000] cursor-pointer" />
      </div>
...
function Step2({ initial, onNext, onBack }: { initial: Partial<FormData>; onNext: (d: z.infer<typeof step2Schema>) => void; onBack: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onNext)} className="space-y-6">
        ...
        <div className="space-y-2">
          {SERVICE_OPTIONS.map((s) => (
            <label key={s.value} className="flex items-center gap-3 p-3.5 rounded-xl border border-[#2C2C2E] bg-[#141414] cursor-pointer hover:border-[rgba(255,255,255,0.14)] transition-colors">
              <input type="checkbox" value={s.value} {...register("services")} className="accent-[#FF0000] w-4 h-4" />
      ...
      {hasPanel && (
        <div className="grid grid-cols-2 gap-3 p-4 bg-[rgba(255,0,0,0.04)] border border-[rgba(255,0,0,0.15)] rounded-xl">
          <div>
            <label className="text-xs text-[#A1A1A6] mb-2 block">Largura do painel (m)</label>
            <input type="number" step="0.5" {...register("panelWidth")} placeholder="Ex: 4" className="w-full bg-[#141414] border border-[#2C2C2E] rounded-lg px-3 py-2.5 text-sm text-white outline-none focus:border-[#FF0000] transition-colors" />
          </div>
          <div>
            <label className="text-xs text-[#A1A1A6] mb-2 block">Altura do painel (m)</label>
            <input type="number" step="0.5" {...register("panelHeight")} placeholder="Ex: 2.5" className="w-full bg-[#141414] border border-[#2C2C2E] rounded-lg px-3 py-2.5 text-sm text-white outline-none focus:border-[#FF0000] transition-colors" />
          </div>
        </div>
      )}
      <div>
        <label className="text-xs text-[#A1A1A6] mb-2 block">Detalhes do projeto (opcional)</label>
        <textarea {...register("details")} rows={3} placeholder="Descreva o evento, local, necessidades específicas..." className="w-full bg-[#141414] border border-[#2C2C2E] rounded-xl px-4 py-3 text-sm text-white placeholder:text-[#6E6E73] outline-none focus:border-[#FF0000] transition-colors resize-none" />
      </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      {[
        ...
        <div key={f.name}>
          ...
          <input {...register(f.name)} placeholder={f.placeholder} className="w-full bg-[#141414] border border-[#2C2C2E] rounded-xl px-4 py-3 text-sm text-white placeholder:text-[#6E6E73] outline-none focus:border-[#FF0000] transition-colors" />
          {errors[f.name] && <p className="text-xs text-[#FF453A] mt-1">{errors[f.name]?.message}</p>}
        </div>
      ))}
      <label className="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" {...register("acceptWhatsApp")} defaultChecked className="accent-[#FF0000] w-4 h-4" />
